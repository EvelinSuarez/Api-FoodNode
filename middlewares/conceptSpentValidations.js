...
const ConceptSpent = require('../models/conceptSpent');  // Cambiar a ConceptSpent

// Validación para asegurarnos de que el concepto de gasto exista
const validateConceptSpentExistence = async (id) => {
    const conceptSpent = await ConceptSpent.findByPk(id);
    if (!conceptSpent) {
...
const updateConceptSpentValidation = [
    ...conceptSpentBaseValidation,
    param('idExpenseType').isInt({ min: 1 }).withMessage('El id debe ser un número entero positivo'),
