...
const changeStateSpecSheet = async (req, res) => {
  // ... (validaciones, obtención de id y status) ...
  try {
    const idSpecsheet = req.params.id;
    const newStatus = req.body.status;

    if (typeof newStatus !== 'boolean') /* ... error ... */ ;

    const result = await specSheetService.changeStateSpecSheet(idSpecsheet, newStatus);

    if (result && result[0] > 0) {
      res.status(200).json({ message: "Estado y fecha de inactividad actualizados correctamente." });
    } else {
      res.status(404).json({ message: "Ficha técnica no encontrada o no se realizaron cambios." });
    }
  } catch (error) {
    // ... manejo de error ...
  }
